...
@section('title', 'SIARAT | Surat Keluar')
        ...
        <div class="card">
            <div class="card-body">
                <div class="table-responsive">
                    <div class="row mb-3">
                        <div class="col-md-4">
                            <a href="{{ route('surat-keluar.create') }}" name="tambah" class="btn btn-primary me-1" role="button" style="color: white;"><i class="fa-solid fa-plus"></i></a>
                        </div>
                        <div class="col-md-8 d-flex justify-content-end">
                            <form action="{{ route('surat-keluar.pdf') }}" method="POST">
                                @csrf
                                <div class="row">
                                    <div class="col-md-4">
                    ...
                    <table class="table table-bordered table-hover nowrap" id="table-suratkeluar" style="width:100%">
                        <thead>
                            <tr>
                                <th>No</th>
                                <th>Nomor Surat</th>
...
$(document).on('click', '.detail_suratkeluar', function(e){
    ...
    $.ajax({
        url: `{{ url('/surat-keluar/${id}') }}`,
        method: 'GET',
        success: function(response){
            window.location.replace(`{{ url('/surat-keluar/${id}') }}`)
        }
    })
})

//Ubah Surat Keluar
$('body').on('click', '.edit_suratkeluar', function(e){
        e.preventDefault()
        const id = $(this).attr('id')
        ...
        $.ajax({
            ...
            method: 'GET',
            success: function(response){
                window.location.replace(`{{ url('surat-keluar/${id}/edit') }}`)
            }
        })
})

//Hapus Surat Masuk
$(document).on('click', '.hapus_suratkeluar', function(e){
        e.preventDefault()
        const id = $(this).attr('id')
        const no_surat = $(this).attr('no_surat')
        const asal_surat = $(this).attr('asal_surat')
        var confirmation = confirm(`Yakin, surat keluar nomor surat ${no_surat}, dari ${asal_surat} akan dihapus?`)
        ...
        if (confirmation) {
            $.ajax({
                ...
                success: function(response){
                    $('#table-suratkeluar').DataTable().ajax.reload()
